        ...
        <div class="allImagesWrapper">
          ...
          <div class="grabber">
            <div class="grabberWrapper">
              <div class="inside">
                <div
                  @blur="checkThereIsFileOrNot($event)"
                  @click="firstImageUpload($event)"
                  id="insideWrappe"
                  v-if="show"
                >
                  <grab-icon></grab-icon>
                ...
                <p v-if="showError" class="error">
                  حداکثر محدودیت اپلود عکس 10 تا میباشد
                </p>
              </div>
            </div>
          </div>
          <div class="w-100 text-center m-auto" v-if="errors.slider">
            <small
              class="text-danger text-right font-weight-bold"
              v-text="errors.slider[0]"
            ></small>
          </div>
        </div>
      </div>
    ...
    async firstImageUpload() {
      ...
      setTimeout(() => {
        const el = document.getElementById(
          `${this.singleImgsArray[this.singleImgsArray.length - 1].id}`
        );
        el.style.display = "block";
        const firstLabel = document.getElementById(
          `${this.singleImgsArray[this.singleImgsArray.length - 1].id}input`
        );
        const firstImage = document.getElementById(
          `${this.singleImgsArray[this.singleImgsArray.length - 1].id}image`
        );
        firstLabel.click();
        this.counter++;
      }, 100);
